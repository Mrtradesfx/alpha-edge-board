...
const SecureSmartAlerts = () => {
  ...
  const handleAddAlert = async () => {
    if (!newAlert.symbol || !newAlert.alert_price || !newAlert.label) return;

    setIsCreating(true);
    const result = await addAlert({
      symbol: newAlert.symbol,
      alert_price: parseFloat(newAlert.alert_price),
      ...
      <Card className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
        ...
        <CardContent className="text-center py-8">
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Sign in to create and manage your secure smart alerts.
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-500">
            Your alerts are stored securely and only accessible to you.
          </p>
        </CardContent>
      </Card>
    );
        ...
        <CardContent className="space-y-6">
          ...
          <Card className="bg-gray-50 dark:bg-gray-800">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewAlert(prev => ({ ...prev, alert_price: e.target.value }))}
                  />
                </div>
              ...
              <Button 
                ...
                disabled={!newAlert.symbol || !newAlert.alert_price || !newAlert.label || isCreating}
                className="w-full"
              >
                {isCreating ? (
                  <>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Your Secure Alerts ({alerts.filter(a => a.is_active).length}/{alerts.length})
            </h3>
            ...
            {isLoading ? (
              ...
              <p className="text-sm text-gray-500 text-center py-8">
                No alerts configured. Create your first secure alert above.
              </p>
            ) : (
              <div className="space-y-2">
                {alerts.map((alert) => (
                  <Card key={alert.id} className={`p-3 ${alert.is_active ? 'bg-green-50 dark:bg-green-950/20' : 'bg-gray-100 dark:bg-gray-800'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                          {alert.direction} {alert.alert_price} ({alert.label})
                        </span>
...
export default SecureSmartAlerts;
